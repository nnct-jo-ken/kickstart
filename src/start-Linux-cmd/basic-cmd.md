...
1. [lsコマンド](#lsコマンド)
2. [mkdirコマンド](#mkdirコマンド)
3. [cdコマンド](#cdコマンド)
4. [touchコマンド](#touchコマンド)
5. [rmコマンド](#rmコマンド)
6. [mvコマンド](#mvコマンド)
7. [catコマンド](#catコマンド)
8. [echoコマンド](#echoコマンド)
9. [grepコマンド](#grepコマンド)
10. [その他のコマンド](#その他のコマンド)

## manコマンド
弟「ねぇねぇ、ﾆｲﾁｬﾝ」  
弟「いろんなコマンドの使い方を簡単に知る方法ないの？」  

ﾆｲﾁｬﾝ「それなら`man`コマンドというのがあるよ」  
ﾆｲﾁｬﾝ「[lsコマンド](#lsコマンド)の使い方をみてみよう！」  

ex.) everywhere

![](../images/man.png)

ﾆｲﾁｬﾝ「`NAME`、`SYNOPSIS`(あらすじ)、`DESCRIPTION`(説明)があるね」  
ﾆｲﾁｬﾝ「全部英語だけど難しくないよ」  
ﾆｲﾁｬﾝ「`man`コマンドは公式の説明を表示してくれるよ」  

弟「へぇ、`man`コマンド使うことで、使い方を調べることができるのか！」  
弟「ありがとうﾆｲﾁｬﾝ」  
...
#### オプション
オプションとは
> オプションとは、機能を追加するかどうかの選択のためのもの

ﾆｲﾁｬﾝ「`ls`はよく多用するよ！」  
ﾆｲﾁｬﾝ「`ls`を使うときによく使うオプションは以下の通りだよ」  
（**あくまでも一例です**）  
...
ﾆｲﾁｬﾝ「`-a`オプションの「全て」というのは、隠しファイルも表示してくれるよ」  

弟「隠しファイルってなに？」  

ﾆｲﾁｬﾝ「隠しファイルっていうのは、フォルダを開いたときに非表示になっているファイルのことだよ」  

ﾆｲﾁｬﾝ「また、`-l`オプションの「詳細」というのは、ファイルサイズや権限などが表示されるよ」  

![](../images/ls-l.png)

ﾆｲﾁｬﾝ「これらのオプションも試してみよう！」  

ﾆｲﾁｬﾝ「オプションはまだあるけど、気になったら調べてみてね」  
...
#### オプション
ﾆｲﾁｬﾝ「`mkdir`でよく使うオプションは以下の通りだよ」  

|オプション|動作                |例                         |
|----------|--------------------|---------------------------|
|-p        |親ディレクトリも作成|mkdir -p jo-ken/prac/sample|


ﾆｲﾁｬﾝ「`-p`オプションを付けることで`jo-ken/prac/sample`というdirectoryを一発で作れるよ」  
ﾆｲﾁｬﾝ「`-p`が無いと、こういう風にdirectoryは作れないんだよ」  

ex.) $HOME/ （`-p`オプションなし）

<img src="../images/mkdir_no-p.png" width=50%>

ex.) $HOME/ (`-p`オプションあり）

<img src="../images/mkdir-p.png" width=50%>
...
ﾆｲﾁｬﾝ「今わからなくても、使ってるうちに覚えられるから大丈夫！！」  
