...
int a;
```
これでaという名前の変数が作られました．`int`は「整数を入れる変数を作れ」という意味です．
...
a=6;
```
`a=6`の行が追加されました．これでaという名前の変数に6という数字が入りました．注意するのは，ここでのイコール(=)は，**等しいという意味ではありません**．イコールの右にある数をaに「**代入する**」という意味です．
...
- 一文字目以外はアルファベット，数字，アンダーバーのみで構成されている
ことがルールです．
...
a=2;
```
[^2]:厳密には，同じブロック内でなければ問題ありませんが，今は気にしなくてよいです．
...
[^3]:整数同士で割り算をすると，小数点以下は切り捨てられます．10/3=3,3/4=0となります．

[^4]:数学でいうと合同式のmodのようなものです．例えば，整数aの一の位はa%10で得ることができます．
...
次のソースコードでは，`a=b`と`b=a`が同じ行にあり，同時に実行するように見えるかもしれませんが，そんなことはありません．`a=b;`のあとに改行したコードと全く同じ意味になります．
...
チャプター2で出てきた顔です．
どうしてもこの顔が気に入らなければ別の絵を用意してもらって構いません．
